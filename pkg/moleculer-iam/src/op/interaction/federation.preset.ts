...
import { OIDCAccountClaims } from "../proxy";
import { Identity, IdentityProvider, Errors } from "../../idp";
...
export type IdentityFederationCallback = (args: { idp: IdentityProvider, profile: Profile, accessToken: string, scope: string[], logger: Logger }) => Promise<Identity | void>;

export type IdentityFederationProviderOptions = {
  kakao?: Partial<Omit<KakaoOptions, "callbackURL"> & { scope: string | string[], callback: IdentityFederationCallback }>,
  google?: Partial<Omit<GoogleOptions, "callbackURL"> & { scope: string | string[], callback: IdentityFederationCallback }>,
  facebook?: Partial<Omit<FacebookOptions, "callbackURL"> & { scope: string | string[], callback: IdentityFederationCallback }>,
} & {
  [key: string]: { scope: string | string[], callback: IdentityFederationCallback, [key: string]: any },
};

export const defaultIdentityFederationProviderStrategies: { [provider: string]: new(opts: any, callback: any) => Strategy } = {
  kakao: KakaoStrategy,
