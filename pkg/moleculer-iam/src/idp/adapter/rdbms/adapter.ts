...
export class IDP_RDBMS_Adapter extends IDPAdapter {
  ...
  private readonly getVersionedClaimsLoader = new DataLoader<{ id: string, claims: { key: string; schemaVersion?: string }[] }, Partial<OIDCAccountClaims>>(
    async (entries) => {
      const where: WhereAttributeHash = {
        id: entries.map(entry => entry.id),
        key: [...new Set(entries.reduce((keys, entry) => keys.concat(entry.claims.map(c => c.key)), [] as string[]))],
      };
      const foundClaimsList: Partial<OIDCAccountClaims>[] = new Array(entries.length).fill(null).map(() => ({}));
